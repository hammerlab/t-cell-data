...
We want to find data about gene expression in T cells. Through analysis of this data, we hope to be able to distinguish T cells from other cells, as well as T cell subsets from one another.

A tibble-centric view of this chapter:

1. `c7_tcells`: T cell subset of ImmuneSigDB. Each row is a gene set.
...
Each row in `c7_tib` represents a gene set from ImmuneSigDB. We filter down to those we suspect relate to T cells, and compute some descriptive statistics about them.
...
c7_tcells %>% group_by(organism) %>% count() # Homo sapiens 612, Mus musculus 1124
# Human T cell publications
c7_tcells %>% filter(organism == 'Homo sapiens') %>% group_by(pmid) %>% count() %>% arrange(desc(n))
```
