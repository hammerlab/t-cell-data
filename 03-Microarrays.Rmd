...
download_dir <- path(data_dir, "series_matrix_files")
...
download_dir <- path(data_dir, "cel_files")
if (!dir_exists(download_dir)) {
  dir_create(download_dir)
  cel_file_names <- path_file(url_parse(cel_file_urls)$path)
  cel_files_local <- map2(cel_file_urls, path(download_dir, cel_file_names), ~ curl_download(.x, .y)) %>% simplify()
} else {
...
download_dir <- path(data_dir, "chip_files")
if (!dir_exists(download_dir)) {
  dir_create(download_dir)
  chip_file <- curl_download(chip_file_url, path(download_dir, chip_file_name))
} else {
  chip_file <- path(download_dir, chip_file_name)
}
```
